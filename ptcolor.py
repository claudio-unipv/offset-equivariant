...
batch of images (Bx3xHxW).  RGB and HSV values are supposed to be in
the range 0-1 (but values outside the range are tolerated).
...
_EPSILON = 0.008856
_KAPPA = 903.3
_XYZ_TO_LAB = _t([[0., 116., 0.], [500., -500., 0.], [0., 200., -200.]])
_LAB_TO_XYZ = _t([[1. / 116., 1. / 500., 0], [1. / 116., 0, 0], [1. / 116., 0, -1. / 200.]])
_LAB_OFF = _t([16.0, 0.0, 0.0]).view(1, 3, 1, 1)


_HSV_DIFF = _t([[0., 1., -1.], [-1., 0., 1.], [1., -1., 0.]])
...
def apply_gamma(rgb, gamma="srgb"):
    ...
    if gamma == "srgb":
        ...
        return torch.where(rgb < T, 12.92 * rgb,
                           (1.055 * torch.pow(torch.abs(rgb1), 1 / 2.4) - 0.055))
    elif gamma is None:
        return rgb
    else:
...
def remove_gamma(rgb, gamma="srgb"):
    ...
    if gamma == "srgb":
        ...
        res = (torch.pow(torch.max(rgb, rgb.new_tensor(0.0)), gamma) +
               torch.min(rgb, rgb.new_tensor(0.0)))  # avoid vanishing gradients
        return res
...
def rgb2xyz(rgb, gamma_correction="srgb", clip_rgb=False, space="srgb"):
    ...
    >>> rgb2xyz(torch.tensor([0.4, 0.8, 0.2]).view(1, 3, 1, 1),
    ...         gamma_correction=None, space='prophoto').view(-1)
...
def rgb2hsv(rgb):
    ...
    >>> rgb = torch.tensor([0.75, 0., 0.]).view(1, 3, 1, 1)
    >>> hsv = rgb2hsv(rgb)
    >>> print(hsv.view(-1))
    tensor([0.0000, 1.0000, 0.7500])

    >>> rgb = torch.tensor([0.5, 0.5, 0.5]).view(1, 3, 1, 1)
    >>> hsv = rgb2hsv(rgb)
    >>> print(hsv.view(-1))
    tensor([0.0000, 0.0000, 0.5000])

    >>> rgb = torch.tensor([0.5, 0.5, 1.0]).view(1, 3, 1, 1)
    >>> hsv = rgb2hsv(rgb)
    >>> print(hsv.view(-1))
    tensor([0.6667, 0.5000, 1.0000])
    """
    m = rgb.max(1, keepdim=True)
    v = m.values
    c = v - rgb.min(1, keepdim=True).values
    s = c / v.clamp(min=1e-5)
    diff = _mul(_HSV_DIFF, rgb)
    off = rgb.new_tensor([0.0, 2.0, 4.0]).view(1, 3, 1, 1)
    h1 = off + diff / c.clamp(min=1e-5)
    h = (torch.gather(h1, 1, m.indices) % 6) / 6
    return torch.cat((h, s, v), 1)


def hsv2rgb(hsv):
    """HSV to RGB conversion.

    >>> hsv = torch.tensor([0.5, 1.0, 1.0]).view(1, 3, 1, 1)
    >>> rgb = hsv2rgb(hsv)
    >>> print(rgb.view(-1))
    tensor([0., 1., 1.])

    >>> hsv = torch.tensor([0.75, 0.0, 0.5]).view(1, 3, 1, 1)
    >>> rgb = hsv2rgb(hsv)
    >>> print(rgb.view(-1))
    tensor([0.5000, 0.5000, 0.5000])

    >>> hsv = torch.tensor([1.0, 0.5, 1.0]).view(1, 3, 1, 1)
    >>> rgb = hsv2rgb(hsv)
    >>> print(rgb.view(-1))
    ...
    # R, G, B = f(0), f(1), f(2)
    # f(n) = V * (1 - g(n)),
    # g(n) = S * max(0, min(k, 4 - k, 1)),
    # k = (5 - 2 * n + 6 * H) mod 6
    n = hsv.new_tensor((0.0, 1.0, 2.0)).view(1, 3, 1, 1)
    k = (5 - 2 * n + 6 * hsv[:, 0:1, :, :]) % 6.0
    g = hsv[:, 1:2, :, :] * torch.minimum(k, 4 - k).clamp(0, 1)
    rgb = hsv[:, 2:3, :, :] * (1 - g)
    return rgb
...
def _check_conversion(**opts):
    ...
    >>> _check_conversion(white_point='d50', gamma_correction=1.8, clip_rgb=False,
    ...                   space='prophoto')
    True
